...
document.getElementById("addUserForm").addEventListener("submit", async function(e) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert("Gagal tambah user: " + (data.message || "Cek field yang dikirim"));
      console.log(data);
    }
  } catch (err) {
...
async function loadUsers() {
  ...
  try {
    ...
    // Urutkan berdasarkan role: admin > designer > user
    const roleOrder = { admin: 1, designer: 2, user: 3 };
...
async function loadOrders() {
  ...
  data.items.forEach(order => {
    const itemHTML = Array.isArray(order.items)
    ...
    container.innerHTML += `
      <div class="border p-4 rounded shadow">
        ...
        <div class="mt-2 flex gap-2 items-center">
          <select onchange="updateOrderStatus('${order.id}', this.value)" class="border rounded p-1 text-sm">
            <option value="pending" ${order.status === 'pending' ? 'selected' : ''}>Pending</option>
            <option value="paid" ${order.status === 'paid' ? 'selected' : ''}>Paid</option>
            <option value="rejected" ${order.status === 'rejected' ? 'selected' : ''}>Rejected</option>
          </select>
          <button onclick="deleteOrder('${order.id}')" class="text-red-600 text-sm underline">Hapus</button>
        </div>
      </div>
    `;
  });
...
async function updateOrderStatus(orderId, status) {
  try {
    const res = await fetch(`${API_URL}/api/collections/orders/records/${orderId}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json"
